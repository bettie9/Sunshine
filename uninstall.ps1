...
function Find-SunshineEntry {
    ...
    foreach ($p in $keyPaths) {
        $hit = Get-ItemProperty -Path $p -ErrorAction SilentlyContinue |
               Where-Object {
                   $_.DisplayName -and
                   $_.DisplayName -match '^Sunshine($|\s|_)' -and
                   $_.Publisher    -and
                   $_.Publisher    -match '^bettie9$'
               } |
...
if (-not $entry) {
    ...
    Write-Host ">> Uninstalling $($entry.DisplayName) $($entry.DisplayVersion) ($($entry.Publisher))" -ForegroundColor Cyan
